...
def main():
    ...
    #args = parse_args()
    video_dir = "video"
    data = pd.read_csv('obid_map_classname_non_negative_prob.txt', sep=".", names=['classname','obid'])
                    ...
                    for d in trackers:
                        ...
                        if all(i >= 0 for i in d):
                            ...
                            if best_class_probabilities > 0.3:
                                ...
                                with open(r'final_out_put_name_frame_bounding_box.txt', 'a+') as f:
                                    f.write(" ".join(map(str, d))+'.'+dict_obid_classname[d[4]]+'.'+ str(d[4]) + "\n")
                                name = 'data\\out_put_tracker_facenet_proba_final_increase_proba\\frame' + str(c) + '.jpg'
